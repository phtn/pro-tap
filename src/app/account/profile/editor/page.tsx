...
import {EditorDropdown} from './_components/form-dropdown'
  ...
  return (
    <div className='h-[calc(100vh-48px)] overflow-hidden'>
      <ScrollArea className='max-w-4xl rounded-4xl md:py-8'>
        <div className='w-full h-fit dark:bg-origin/80 bg-origin/30 p-4'>
          <div className='mb-2 flex items-center justify-between'>
            <h1 className='flex items-center text-lg md:text-2xl font-bold tracking-tight'>
              <Icon name='draw' className='size-6 shrink-0 mr-2' /> Editor
            </h1>
            <EditorDropdown setCurrentForm={setCurrentForm}>
              <SexyButton rightIcon='chevron-down' className='capitalize'>
                {currentForm}
              </SexyButton>
            </EditorDropdown>
          </div>
          ...
          <div
            className={cn(
              `border mb-3 md:mb-6 px-2 md:px-4 flex items-center w-full h-7 md:h-12 text-xs md:text-sm rounded-lg md:rounded-xl bg-green-100 text-green-700`,
              {
                'bg-red-100 text-red-700': formMessage.includes('Error'),
                'bg-orange-100 text-orange-700':
                  formMessage.includes('invalid'),
                'opacity-80': !formMessage,
              },
            )}>
            {formMessage} Error: message is here.
          </div>
