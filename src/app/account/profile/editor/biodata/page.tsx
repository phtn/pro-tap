...
export default function BioDataPage() {
  ...
  }, [form.state.errors, form.state.isDirty, form.state.isValid])
  ...
  const ValidationSummary = useCallback(
    ...
    [formErrors, isDirty, isValid],
  )

  return (
    <form onSubmit={handleFormSubmit} className='w-full'>
      <div className='h-fit w-full grid grid-cols-1 md:grid-cols-5 gap-4 md:gap-8 md:py-8 max-w-6xl'>
        <div className='rounded-4xl border border-origin bg-white dark:bg-origin col-span-5 md:col-span-3 h-fit'>
            ...
            <div className='mb-4 md:mb-8'>
              {fieldGroups.map((group) => (
                <HyperList
                  ...
                  container='space-y-4 md:space-y-8 py-1'
                  itemStyle='px-1'
                />
              ))}
            </div>
        ...
        <div className='rounded-4xl bg-white border border-origin dark:bg-origin col-span-5 md:col-span-2 h-full'>
          <div className='mb-4 md:mb-8'>
            <ScrollArea className='w-full px-4 py-4 md:py-8'>
              ...
              {fieldGroups.map((group) => (
                <HyperList
                  ...
                  component={renderField}
                  container='space-y-4 md:space-y-12 py-1'
                  itemStyle='px-1'
                />
              ))}
            </ScrollArea>
