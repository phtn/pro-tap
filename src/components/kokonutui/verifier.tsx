...
export function Verifier({
  ...
  const XX = useCallback(() => {
    const options = opts(<Y />, <V isCompleted={isCompleted} />)
    return (
      <AnimatePresence mode='wait'>{options.get(isCompleted)}</AnimatePresence>
    )
  }, [isCompleted])

  const ZZ = useCallback(() => {
    const options = opts(<S />, <T />)
    return (
      <AnimatePresence mode='wait'>{options.get(isCompleted)}</AnimatePresence>
    )
  }, [isCompleted])

  const WW = useCallback(() => {
    const options = opts(<H isCompleted={isCompleted} />, <J code={code} />)
    return (
      <AnimatePresence mode='wait'>
        <div className='flex items-center justify-center'>
          {options.get(isCompleted)}
        </div>
      </AnimatePresence>
    )
  }, [isCompleted, code])
...
const H = ({isCompleted}: CompletedProps) => {
  return (
    <motion.div
      key='completed-id'
      className={cn(
...
const V = ({isCompleted}: CompletedProps) => {
  return (
    <motion.div
      key='progress'
      initial={{opacity: 0}}
