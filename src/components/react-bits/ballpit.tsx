...
import type { Material, Mesh, MeshPhysicalMaterialParameters } from 'three'
import {
...
class X {
  #config: XConfig
  #postprocessing: PostProcessingLike | undefined
  ...
  set postprocessing(value: PostProcessingLike) {
  ...
  clear() {
    this.scene.traverse((obj) => {
      if ('isMesh' in obj && (obj as unknown as { isMesh: boolean }).isMesh) {
...
class Y extends MeshPhysicalMaterial {
  ...
  constructor(params: MeshPhysicalMaterialParameters = {}) {
    ...
    this.onBeforeCompile = (
      ...
      _renderer: Parameters<Material['onBeforeCompile']>[1],
    ) => {
      // Ensure UVs are available by defining USE_UV on the shader just before compile
      if (!(shader as unknown as { defines?: Record<string, string> }).defines) {
        ; (shader as unknown as { defines?: Record<string, string> }).defines = {}
      }
      ; (shader as unknown as { defines: Record<string, string> }).defines.USE_UV =
        ''
...
function createBallpit(
  ...
  function initialize(cfg: Partial<typeof defaultBallpitConfig>) {
    if (spheres) {
      threeInstance.clear()
...
interface BallpitProps extends Partial<typeof defaultBallpitConfig> {
  className?: string
  followCursor?: boolean
}
