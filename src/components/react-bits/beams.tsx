...
type UniformScalar = number | boolean | string;
type UniformComplex =
  | THREE.Color
  | THREE.Vector2
  | THREE.Vector3
...
type UniformLike = { value: unknown } | UniformScalar | UniformComplex | undefined;

type ShaderDefines = Record<string, number | string | boolean>;
...
type ExtendMaterialConfig = {
  ...
  material?: Pick<THREE.ShaderMaterialParameters, "fog">;
  uniforms?: Record<string, UniformLike>;
};

function extendMaterial(
  BaseMaterial: typeof THREE.MeshStandardMaterial = THREE.MeshStandardMaterial,
  ...
  const defaults = new BaseMaterial({} as THREE.MeshStandardMaterialParameters);
  ...
  if ("roughness" in defaults)
    uniforms.roughness.value = (defaults as THREE.MeshStandardMaterial).roughness;
  if ("metalness" in defaults)
    uniforms.metalness.value = (defaults as THREE.MeshStandardMaterial).metalness;
  if ("envMap" in defaults)
    uniforms.envMap.value = (defaults as THREE.MeshStandardMaterial).envMap;
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    uniforms[key] =
      typeof u === "object" && u !== null && "value" in (u as object)
        ? (u as { value: unknown })
        : { value: u as unknown };
  });
...
function createStackedPlanesBufferGeometry(
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      const v0: [number, number, number] = [xOffset, y, 0];
      const v1: [number, number, number] = [xOffset + width, y, 0];
